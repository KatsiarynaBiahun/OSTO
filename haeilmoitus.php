...
if((isset($_POST['submit'])) AND (!empty($haku))) { 
    ...
    if ($num == 0) { 
        ...
        while ($i < $num) { 
            ...
            Ilmoitus jätetty: $ilmoitus_oikeapaivays</td></tr><tr><td>Ilmoittaja: 
            $myyja_tunnus (<a href='mailto: $myyja_sahkoposti'>$myyja_sahkoposti</a>)</td></tr>"; 

            if ((isset($_SESSION['kayttaja_id']) && $_SESSION['kayttaja_id'] == 
            $myyja_id) or (isset($_SESSION['kayttaja_taso']) && 
            $_SESSION['kayttaja_taso'] == "admin")) { 
                echo "<tr><td><form action='poistailmoitus.php' method='post'/><input type='hidden' name='poista' 
                value='1'><input type='hidden' name='poista_id' value='$ilmoitus_id'><input type='submit' 
                value='Poista'></form><form action='muokkaailmoitus.php' method='post'/><input type='hidden' name='muokkaa_id' 
                value='$ilmoitus_id'/><input type='submit' value='Muokkaa'/></form></td></tr>"; 
            } 
